...
// Pokémon high-level config
const MAX_POKEMON_ID = 1025; // Updated to include newer Pokémon
const SHINY_RATE = 0.1; // Increased for more fun
...
const fetchPokemonData = async (
  ...
  if (
    ...
    if (specialVarieties.length > 0) {
      const variety =
        specialVarieties[Math.floor(Math.random() * specialVarieties.length)];
  ...
  const jaName =
    speciesData.names.find((n: any) => n.language.name === "ja")?.name ||
    data.name;
  const jaGenus =
    speciesData.genera.find((g: any) => g.language.name === "ja")?.genus || "";
  const jaFlavorTextEntry =
    speciesData.flavor_text_entries.find(
      (f: any) => f.language.name === "ja-Hrkt",
    ) ||
    speciesData.flavor_text_entries.find((f: any) => f.language.name === "ja");

  const jaFlavorText = jaFlavorTextEntry
    ? jaFlavorTextEntry.flavor_text
    : speciesData.flavor_text_entries.find((f: any) => f.language.name === "en")
        ?.flavor_text || "";

  const jaTypes = data.types.map(
    (t: any) => TYPE_NAME_MAP[t.type.name] || t.type.name,
  );
  ...
  return {
    ...
    isShiny,
    cry: data.cries?.legacy || data.cries?.latest, // Use legacy for better iOS compatibility
...
const fetchQuizData = async (
  ...
  if (category === "type") {
    ...
    Object.entries(TYPE_CHART).forEach(([attacker, strongAgainst]) => {
      ...
      const isStrong = correctPokemon.types?.some((typeJa) => {
        const defTypeEn = Object.entries(TYPE_NAME_MAP).find(
          ([_, ja]) => ja === typeJa,
        )?.[0];
    ...
    while (wrongChoices.length < 3) {
      const randomType =
        allTypesJa[Math.floor(Math.random() * allTypesJa.length)];
      if (
        !weaknesses.includes(randomType) &&
        !wrongChoices.includes(randomType)
      ) {
        wrongChoices.push(randomType);
      }
    }

    const choices = [correctAnswer, ...wrongChoices].sort(
      () => Math.random() - 0.5,
    );
  ...
  const wrongNames = await Promise.all(
    wrongIds.map(async (id) => {
      ...
      const data = await res.json();
      return (
        data.names.find((n: any) => n.language.name === "ja")?.name || "Unknown"
      );
    }),
  );

  const choices = [...wrongNames, correctPokemon.name].sort(
    () => Math.random() - 0.5,
  );
...
function App() {
  ...
  const [totalCorrectCount, setTotalCorrectCount] = useState(
    () => Number(localStorage.getItem("totalCorrectCount")) || 0,
  );
  ...
  const [timeAttackBestScores, setTimeAttackBestScores] = useState<
    Record<number, number>
  >(() =>
    JSON.parse(
      localStorage.getItem("timeAttackBestScores") ||
        '{"30": 0, "60": 0, "120": 0}',
    ),
  );
  ...
  const [activeTab, setActiveTab] = useState<
    "home" | "speed" | "book" | "settings"
  >("home");

  // Shiritori states
  const [isShiritori, setIsShiritori] = useState(false);
  const [shiritoriHistory, setShiritoriHistory] = useState<
    { sender: "player" | "ai"; name: string; image?: string }[]
  >([]);
  const [usedPokemonNames, setUsedPokemonNames] = useState<Set<string>>(
    new Set(),
  );
  const [allPokemonDatabase, setAllPokemonDatabase] = useState<
    { name: string; id: number }[]
  >([]);
  const [shiritoriInput, setShiritoriInput] = useState("");
  const [isAiThinking, setIsAiThinking] = useState(false);
  const [shiritoriWinner, setShiritoriWinner] = useState<
    "player" | "ai" | null
  >(null);
  ...
  const startShiritori = async () => {
    ...
    setTimeout(() => {
      ...
      const candidates = allPokemonDatabase.filter(
        (p) => !p.name.endsWith("ン"),
      );
      const aiChoice =
  ...
  const handleShiritoriSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const candidates = allPokemonDatabase.filter(
        (p) => p.name.startsWith(targetChar) && !newUsed.has(p.name),
      );
      ...
      if (!aiChoice)
        aiChoice = candidates[Math.floor(Math.random() * candidates.length)];
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: "1.25rem", lineHeight: 1 }}>
            {getLevelInfo(totalCorrectCount).level}
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <div
  ...
  const renderBookTab = () => (
    ...
    >
      {renderHeader(
        "📖 ポケモンずかん",
        `${caughtPokemon.length} ぴき つかまえたよ！`,
      )}
          ...
          {THEMES.map((t) => (
            <button
              ...
              style={{
                ...
                boxShadow:
                  theme === t.id
                    ? "0 0 0 2px var(--bg-panel), 0 4px 8px rgba(0,0,0,0.2)"
                    : "none",
                ...
                (gen) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "8px",
                      background:
                        selectedGen === gen ? undefined : "rgba(0,0,0,0.03)",
                      color:
                        selectedGen === gen ? "white" : "var(--text-primary)",
                      border:
                        selectedGen === gen
                          ? "none"
            ...
            >
              <button
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  background:
                    selectedType === "all" ? undefined : "rgba(0,0,0,0.03)",
                  color:
                    selectedType === "all" ? "white" : "var(--text-primary)",
                  border:
                    selectedType === "all"
                      ? "none"
                      : "1px solid var(--border-color)",
              ...
              {Object.entries(TYPE_NAME_MAP).map(([en, ja]) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "8px",
                    background:
                      selectedType === en ? undefined : "rgba(0,0,0,0.03)",
                    color:
                      selectedType === en ? "white" : "var(--text-primary)",
                    border:
                      selectedType === en
                        ? "none"
                        : "1px solid var(--border-color)",
    ...
    return (
      <div className="app-container">
        ...
        <div className="tab-bar">
          ...
          <button
            className={`tab-item ${activeTab === "speed" ? "active" : ""}`}
  ...
  if (isShiritori) {
    const lastWord =
      shiritoriHistory.length > 0
        ? shiritoriHistory[shiritoriHistory.length - 1].name
        : "";
        ...
        <div className="main-content">
          ...
          >
            ...
            {shiritoriLoading ? (
              ...
              >
                <div className="pulse" style={{ fontSize: "3rem" }}>
                  🔍
                </div>
                ...
                >
                  <div
                    style={{
                      height: "100%",
                      background:
                        "linear-gradient(90deg, var(--primary-color), #DA70D6)",
                      width: `${Math.min(100, (allPokemonDatabase.length / 1025) * 100)}%`,
                      transition: "width 0.3s",
                    }}
                  ></div>
                </div>
                ...
                >
                  {allPokemonDatabase.length} / 1025
                </p>
              </div>
            ) : (
              <>
                  ...
                  {shiritoriHistory.map((chat, i) => (
                    <div
                      ...
                      style={{
                        display: "flex",
                        justifyContent:
                          chat.sender === "player" ? "flex-end" : "flex-start",
                        alignItems: "center",
                        gap: "0.5rem",
                      }}
                    >
                      ...
                      <div
                        style={{
                          ...
                          borderRadius: "18px",
                          borderTopLeftRadius:
                            chat.sender === "ai" ? "4px" : "18px",
                          borderTopRightRadius:
                            chat.sender === "player" ? "4px" : "18px",
                          background:
                            chat.sender === "player"
                              ? "var(--primary-color)"
                              : "white",
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          type="submit"
                          className="btn-primary"
                          disabled={!shiritoriInput.trim() || isAiThinking}
      ...
      <div className="main-content">
        ...
        <div className="pokemon-display-area">
          <div style={{ position: "relative" }}>
            ...
            {displayMode === "cry" && !showResult ? (
              <button
                onClick={playCry}
                className="bounce-in btn-secondary"
                style={{
                  width: "200px",
                  height: "200px",
                  borderRadius: "50%",
        ...
        {!showResult && hintLevel > 0 && (
          ...
          >
            💡{" "}
            {hintLevel === 1
              ? `タイプ: ${currentPokemon.types?.join(" / ")}`
              : `ヒント: ${currentPokemon.name[0]}...`}
          </div>
        )}
            ...
            >
              ...
              >
                <section>
                  ...
                  >
                    <span style={{ fontSize: "1.2rem" }}>🎮</span> クイズの
                    モード
                  </h3>
                  <div
                    style={{
                ...
                <section>
                  ...
                  >
                    せいかいするたびに けいけんちが たまるよ！
                    <br />
                    5もん せいかいするごとに <strong>レベルアップ！</strong>
                    <br />
                ...
                <section>
                  ...
                  >
                    <span style={{ fontSize: "1.2rem" }}>📱</span> アプリにする
                    (PWA)
                  </h3>
                  <div
                    style={{
